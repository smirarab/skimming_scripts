...
mkdir -p ${out_dir}/skim_processing_single_no_merge/bbmap_${genome}
cd ${out_dir}/skim_processing_single_no_merge/bbmap_${genome}

echo "BBMap cleanup pipeline starts"

${ref_dir}/bbmap_pipeline_nm.sh ${input_1} ${genome}.fq
...
skmer --debug query -a -p ${cores} ${out_dir}/skim_processing_single_no_merge/bbmap_${genome}/${genome}.fq ${lib_dir}
rm dist-${genome}.txt
echo "Skmer query done; Skmer distance running"
skmer distance ${lib_dir}
echo "Skmer distance done"

mkdir -p ${out_dir}/skim_processing_single_no_merge/respect_${genome}/{tmp,output}

cov_val=$(grep coverage ${lib_dir}/${genome}/${genome}.dat |cut -f2)
...
then
	...
	${genome}.hist $(grep read_length ${lib_dir}/${genome}/${genome}.dat |cut -f2)" >${out_dir}/skim_processing_single_no_merge/respect_${genome}/info.txt

	respect --debug -N ${iterations} --threads ${threads} -I ${out_dir}/skim_processing_single_no_merge/respect_${genome}/info.txt -i ${lib_dir}/${genome}/${genome}.hist --tmp ${out_dir}/skim_processing_single_no_merge/respect_${genome}/tmp -o ${out_dir}/skim_processing_single_no_merge/respect_${genome}/output
	echo "Respect done"
else
        ratio_val="$( echo "scale=4; $set_bound / $cov_val" | bc)"
        echo "Coverage not in range, sample to be downsampled by ${ratio_val}"
        seqtk sample ${out_dir}/skim_processing_single_no_merge/bbmap_${genome}/${genome}.fq $ratio_val> ${out_dir}/skim_processing_single_no_merge/respect_${genome}/downsampled_${genome}_${ratio_val}.fq
        echo "Running respect on downsampled sample"
        respect -i ${out_dir}/skim_processing_single_no_merge/respect_${genome}/downsampled_${genome}_${ratio_val}.fq -N 200 --debug --tmp ${out_dir}/skim_processing_single_no_merge/respect_${genome}/tmp -o ${out_dir}/skim_processing_single_no_merge/respect_${genome}/output
	echo "Respect done"       
