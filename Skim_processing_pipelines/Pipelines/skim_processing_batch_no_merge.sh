...
mkdir -p ${out_dir}/skim_processing_batch_no_merge/bbmap
mkdir -p ${out_dir}/skim_processing_batch_no_merge/respect/{tmp,output}
...
for f in `ls -d ${input}/*`;do
	...
	cd ${out_dir}/skim_processing_batch_no_merge/bbmap
	
	echo "BBMap cleanup pipeline starts"
 
 	${ref_dir}/bbmap_pipeline_nm.sh ${input_sample} ${genome}_merged.fq
 	echo "BBMap cleanup pipeline ends"
 
 	cd ${ref_dir}
	
	echo "Skmer query running"
	skmer --debug query -a -p ${cores} ${out_dir}/skim_processing_batch_no_merge/bbmap/${genome}_merged.fq ${lib_dir}
	...
	then
        	...
        	${genome}_merged.hist $(grep read_length ${lib_dir}/${genome}_merged/${genome}_merged.dat |cut -f2)" >${out_dir}/skim_processing_batch_no_merge/respect/info.txt

        	respect --debug -N ${iterations} --threads ${threads} -I ${out_dir}/skim_processing_batch_no_merge/respect/info.txt -i ${lib_dir}/${genome}_merged/${genome}_merged.hist --tmp ${out_dir}/skim_processing_batch_no_merge/respect/tmp -o ${out_dir}/skim_processing_batch_no_merge/respect/output
        	echo "Respect done"
		rm ${out_dir}/skim_processing_batch_no_merge/respect/info.txt
	else
        	ratio_val="$( echo "scale=4; $set_bound / $cov_val" | bc)"
        	echo "Coverage not in range, sample to be downsampled by ${ratio_val}"
        	seqtk sample ${out_dir}/skim_processing_batch_no_merge/bbmap/${genome}_merged.fq $ratio_val> ${out_dir}/skim_processing_batch_no_merge/respect/downsampled_${genome}_${ratio_val}.fq
        	echo "Running respect on downsampled sample"
        	respect -i ${out_dir}/skim_processing_batch_no_merge/respect/downsampled_${genome}_${ratio_val}.fq -N 200 --debug --tmp ${out_dir}/skim_processing_batch_no_merge/respect/tmp -o ${out_dir}/skim_processing_batch_no_merge/respect/output
        	echo "Respect done"       
